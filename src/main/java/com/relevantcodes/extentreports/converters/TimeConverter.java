...
public class TimeConverter {
	...
	if (totalTimeTakenOverall != null && totalTimeTakenOverall.size() > 0
		&& !totalTimeTakenOverall.first().text().trim().equals("")) {
	    time = totalTimeTakenOverall.first().text();
	} else if (totalTimeTakenCurrent != null && totalTimeTakenCurrent.size() > 0
		&& !totalTimeTakenCurrent.first().text().trim().equals("")) {
