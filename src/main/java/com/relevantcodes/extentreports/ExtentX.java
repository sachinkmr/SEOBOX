...
public class ExtentX extends LogSettings implements IReporter {
	...
	public void flush() {
		...
		reportCollection.updateOne(new Document("_id", reportId), new Document("$set", new Document("endTime", new Date(report.getSuiteTimeInfo().getSuiteEndTimestamp())).append("status", report.getStatus().toString())));
	}
	...
	private void insertReport() {
		...
		// if extent is started with [replaceExisting = false] and ExtentX is
		// used,
		// use the same report ID for the 1st report run and update the database
		// for
		...
		Document doc = new Document("project", projectId).append("fileName", new File(report.getFilePath()).getName()).append("startTime", report.getStartedTime());
	...
	public void addTest(Test test) {
		...
		Document doc = new Document("report", reportId).append("name", testName).append("status", test.getStatus().toString()).append("description", test.getDescription()).append("startTime", test.getStartedTime()).append("endTime", test.getEndedTime()).append("childNodesCount", getNodeLength(test, 0)).append("categorized", test.getCategoryList().size() > 0 ? true : false);
	...
	private void addLogs(Test test, ObjectId testId) {
		...
		while (iter.hasNext()) {
			...
			doc = new Document("test", testId).append("report", reportId).append("testName", test.getName()).append("logSequence", ix++).append("status", log.getLogStatus().toString()).append("timestamp", log.getTimestamp()).append("stepName", log.getStepName()).append("details", log.getDetails());
	...
	private void addNode(Test node, ObjectId testId, int level) {
		Document doc = new Document("test", testId).append("parentTestName", testName).append("report", reportId).append("name", node.getName()).append("level", level).append("status", node.getStatus().toString()).append("description", node.getDescription()).append("startTime", node.getStartedTime()).append("endTime", node.getEndedTime()).append("childNodesCount", node.getNodeList().size());
	...
	private void addCategories(Test test, ObjectId testId) {
		...
		while (categories.hasNext()) {
			...
			Document doc = new Document("tests", testId).append("report", reportId).append("name", category.getName()).append("status", test.getStatus().toString()).append("testName", test.getName());
			...
			/*
			 * create association with category tests (many) <-> categories
			 * (many) tests and categories have a many to many relationship - a
			 * test can be assigned with one or more categories - a category can
			 * have one or more tests
			 */
			doc = new Document("test_categories", testId).append("category_tests", categoryId).append("category", category.getName()).append("test", test.getName());
	...
	private void addAuthors(Test test, ObjectId testId) {
		...
		while (authors.hasNext()) {
			...
			Document doc = new Document("tests", testId).append("report", reportId).append("name", author.getName()).append("status", test.getStatus().toString()).append("testName", test.getName());

			authorCollection.insertOne(doc);

			ObjectId authorId = MongoUtil.getId(doc);

			doc = new Document("test_authors", testId).append("author_tests", authorId).append("author", author.getName()).append("test", test.getName());
