...
public class ExtentX extends LogSettings implements IReporter {
	...
	reportCollection.updateOne(new Document("_id", reportId),
		new Document("$set", new Document("endTime", new Date(report.getSuiteTimeInfo().getSuiteEndTimestamp()))
	...
	Document doc = new Document("project", projectId).append("fileName", new File(report.getFilePath()).getName())
	...
	Document doc = new Document("report", reportId).append("name", testName)
		.append("status", test.getStatus().toString()).append("description", test.getDescription())
		.append("startTime", test.getStartedTime()).append("endTime", test.getEndedTime())
	...
	while (iter.hasNext()) {
	    ...
	    doc = new Document("test", testId).append("report", reportId).append("testName", test.getName())
		    .append("logSequence", ix++).append("status", log.getLogStatus().toString())
		    .append("timestamp", log.getTimestamp()).append("stepName", log.getStepName())
	...
	Document doc = new Document("test", testId).append("parentTestName", testName).append("report", reportId)
		.append("name", node.getName()).append("level", level).append("status", node.getStatus().toString())
		.append("description", node.getDescription()).append("startTime", node.getStartedTime())
		.append("endTime", node.getEndedTime()).append("childNodesCount", node.getNodeList().size());
	...
	while (categories.hasNext()) {
	    ...
	    Document doc = new Document("tests", testId).append("report", reportId).append("name", category.getName())
		    .append("status", test.getStatus().toString()).append("testName", test.getName());
	     ...
	     */
	    doc = new Document("test_categories", testId).append("category_tests", categoryId)
		    .append("category", category.getName()).append("test", test.getName());
	...
	while (authors.hasNext()) {
	    ...
	    Document doc = new Document("tests", testId).append("report", reportId).append("name", author.getName())
		    .append("status", test.getStatus().toString()).append("testName", test.getName());
	    ...
	    doc = new Document("test_authors", testId).append("author_tests", authorId)
		    .append("author", author.getName()).append("test", test.getName());
