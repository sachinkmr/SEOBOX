 ...
 * 
 * <p>
 * By default, each started node is top-level. If <code>appendChild</code>
 * method is used on any test, it automatically becomes a child-node. When this
 * happens:
 * 
 * <ul>
...
public class ExtentTest implements IExtentTestClass, Serializable {
	 ...
	 * 
	 * <p>
	 * Note: when a test is started using
	 * <code>extent.startTest(testName)</code>, the value for started time is
	 * created automatically. This method allows overriding the start time in
	 * cases where the actual test had already been run prior to extent logging
	 * the test details in the report. An example of this scenario is while
	 * using TestNG's <code>IReporter</code> listener and creating the report
	 * after the tests have already executed.
	 * 
	 * @param startedTime
	 *            Test's start time
	 */
	 ...
	 *
	 * @return A formed HTML img tag
	 */
	@Override
	public String addScreenCapture(String imagePath) {
		String screenCaptureHtml = isPathRelative(imagePath) ? ScreenshotHtml.getSource(imagePath).replace("file:///", "") : ScreenshotHtml.getSource(imagePath);
	...
	public String addScreencast(String screencastPath) {
		String screencastHtml = isPathRelative(screencastPath) ? ScreencastHtml.getSource(screencastPath).replace("file:///", "") : ScreencastHtml.getSource(screencastPath);
	 ...
	 * 
	 * <p>
	 * Usage: <code>test.assignCategory("ExtentAPI");</code> <br>
	 * Usage: <code>test.assignCategory("ExtentAPI", "Regression", ...);</code>
	 * 
	 * @param categories
	 *            Category name
	 * 
	 * @return A {@link ExtentTest} object
	 */
	@Override
	 ...
	 * 
	 * <p>
	 * Usage: <code>test.assignAuthor("AuthorName");</code> <br>
	 ...
	 * 
	 * @return An {@link ExtentTest} object. Parent test which adds the node as
	 *         its child.
	 */
	@Override
	...
	/**
	 * <p>
	 * Returns the interface that exposes some important methods of the
	 * underlying test
	 * 
	 * @return A {@link ITest} object
	 */
	@Override
