...
public class HttpRequestUtils {
	...
	String add = URLCanonicalizer.getCanonicalURL(data[0]);
	Request request = Request.Get(add).addHeader("user-agent", SEOConfig.USER_AGENT)
		.connectTimeout(
			Integer.parseInt(SEOConfig.PROPERTIES.getProperty("crawler.connectionTimeout", "120000")))
		.socketTimeout(
			Integer.parseInt(SEOConfig.PROPERTIES.getProperty("crawler.connectionTimeout", "120000")));
	...
	try {
	    ...
	    // localContext.setCookieStore(getCookies(SEOConfig.PAGE_STRUCTURE_URL));
	    HttpPost httpPost = new HttpPost(SEOConfig.PAGE_STRUCTURE_URL + "/validate");
	...
	try {
	    CloseableHttpClient client = HttpClients.createDefault();
	    URIBuilder builder = new URIBuilder("https://www.googleapis.com/pagespeedonline/v3beta1/runPagespeed");
    ...
    // private static CookieStore getCookies(String pageStructureUrl)
    ...
    // HttpGet httpget = new HttpGet(pageStructureUrl);
    // httpget.addHeader("user-agent", SEOConfig.USER_AGENT);
    // httpget.addHeader("x-client-data", "CIW2yQEIpLbJAQjBtskB");
