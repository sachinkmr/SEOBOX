...
public class PageLevel {
	...
	for (File file : pages) {
		...
		if (page.getPage().getWebURL().isInternalLink() && page.getPage().getStatusCode() == 200
			...
			for (Element e : list) {
				...
				test.log(LogStatus.PASS,
					"<b>Page URL: </b>" + page.getPage().getWebURL().getURL()
	...
	for (File file : pages) {
		...
		if (page.getPage().getWebURL().isInternalLink() && page.getPage().getStatusCode() == 200
		    ...
		    if (links.isEmpty()) {
			test.log(LogStatus.FAIL,
				"<b>URL: </b>" + page.getPage().getWebURL().getURL(),"Canonical tag is missing");
		    } else {
			test.log(LogStatus.PASS,
				"<b>URL: </b>" + page.getPage().getWebURL().getURL(),"Canonical tag found.");
		    }
		}
	    } catch (Exception e) {
		logger.debug("Error " + e);
