...
/**
 * An abstraction of an actual sitemap. This class contains all information that
 * are logically relevant for a sitemap. Class is immutable.
 * 
 * @author Bithazard
 */
public final class Sitemap {
	 ...
	 * 
	 * @param sitemapIndexes
	 *            The sitemap indexes if the sitemap if the sitemap is of type
	 *            INDEX. An empty Set otherwise.
	 * @param sitemapEntries
	 *            All sitemap entries of the sitemap or of all referenced
	 *            sitemaps of a sitemap index
	 * @param sitemapType
	 *            The type of the sitemap. Must not be null
	...
	public enum SitemapType {
		/**
		 * Sitemap of type text. A file that only contains URLs (each in a new
		 * line).
		 */
		TEXT,
		/**
		 * Sitemap of type XML. A file that contains an XML structure which
		 * holds, aside from the URL, other information like the last modified
		 * date of the URL.
		 */
		XML,
		/**
		 * A sitemap index. A file that contains an XML structure in which
		 * sitemaps of type XML are referenced.
		 */
		INDEX;
	}
	 ...
	 * 
	 * @return a Set which contains a SitemapIndex object for each sitemap index
	 *         that was referenced in a parsed sitemap. Returns an empty Set for
	 *         sitemaps of type XML or INDEX.
	 */
	 ...
	 * 
	 * @return a Set which contains a SitemapEntry object for each entry from a
	 *         parsed sitemap. When parsing a sitemap index, all entries from
	 *         all referenced sitemaps are returned.
	 */
	 ...
	 * 
	 * @param url
	 *            The URL of the sitemap entries that should remain in the
	 *            returned sitemap
	 * @return a copy of this sitemap containing only sitemap entries whose URL
	 *         starts with the passed URL. The comparison is case insensitive.
	 */
	...
	/**
	 * Get a sitemap that contains only entries and indexes that were modified
	 * after the passed date. Note: You can also pass a date to the parseSitemap
	 * method of the SitemapParser.
	 * 
	 * @param minLastMod
	 *            The date after which entries and indexes must be modified to
	 *            remain in the returned sitemap
	 * @return a copy of this sitemap containing only sitemap entries and
	 *         sitemap indexes whose last modified date is after the passed
	 *         date.
	 * @see bithazard.sitemap.parser.SitemapParser#parseSitemap(java.io.InputStream,
	 *      java.net.URL, java.util.Date)
	 * @see bithazard.sitemap.parser.SitemapParser#parseSitemap(String,
	 *      java.util.Date)
	 * @see bithazard.sitemap.parser.SitemapParser#parseSitemap(java.net.URL,
	...
	/**
	 * Get a sitemap that contains only entries with one of the passed change
	 * frequencies.
	 * 
	 * @param changeFrequencies
	 *            A set containing one or multiple change frequencies of sitemap
	 *            entries that should remain in the returned sitemap
	 * @return a copy of this sitemap containing only sitemap entries whose
	 *         change frequency is one of the passed change frequencies.
	 */
	...
	/**
	 * Get a sitemap that contains only entries with at least the passed
	 * priority.
	 * 
	 * @param priority
	 *            The priority that a sitemap entry must have to remain in the
	 *            returned sitemap
	 * @return a copy of this sitemap containing only sitemap entries with at
	 *         least the passed priority.
	 */
	 ...
	 * 
	 * @return a string representation of the fields sitemapType, sitemapIndexes
	 *         and sitemapEntries.
	 */
	@Override
	public String toString() {
		return "Sitemap{" + "sitemapType=" + sitemapType + ", sitemapIndexes=" + sitemapIndexes + ", sitemapEntries=" + sitemapEntries + '}';
	}
}
