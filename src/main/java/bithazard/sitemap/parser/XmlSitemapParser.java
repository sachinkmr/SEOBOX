...
class XmlSitemapParser implements ISitemapParser {
	...
	public void parse() {
		...
		try {
			// We want the default JDK XML parser (which is even in the openJDK)
			// by default as the Xerces XML parser does
			// not seem to be able to parse certain XML files. This is only
			// relevant when the Apache Xerces library is
			// also in the classpath and would be chosen first otherwise. If
			// this fails we revert back to the default
	...
	private static class XmlSitemapHandler extends DefaultHandler {
		...
		private String getLastLoc() {
			...
			// well yeah... get the loc of the last SitemapEntry of the Set
			// sitemapEntries
			// probably not a very good performance, but this method is only
			// called in case of an exception anyway
