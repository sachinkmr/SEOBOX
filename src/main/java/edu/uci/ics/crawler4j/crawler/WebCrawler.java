	...
	private void processPage(WebURL curURL) {
		...
		try {
			...
			if (null == prop || prop.isEmpty() || prop.equals(".")) {
				if (!CrawlerConstants.ASSETS_PATTERN.matcher(curURL.getURL()).find() && !curURL.isInternalLink()) {
					throw new ExternalLinkException(Level.WARN, "Skipping parsing of external link: " + curURL);
				}
			} else {
				if (!CrawlerConstants.ASSETS_PATTERN.matcher(curURL.getURL()).find() && !m.find()) {
					throw new ExternalLinkException(Level.WARN, "Skipping parsing of link based on user regex: " + curURL);
				}
			}
